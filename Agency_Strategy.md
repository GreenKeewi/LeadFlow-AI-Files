...
```mermaid
graph TD
    A[Social Media User] -->|Scrolls & Sees| B(AI Video Ad)
    B -->|Clicks 'Get Quote'| C{Lead Capture Form}
    C -->|Submits Contact Info| D[Lead Database / CRM]
    
    subgraph "The AI Engine (Your Value)"
    D --> E[Instant AI Text Outreach]
    E -->|No Response| F[AI Follow-up Sequence]
    E -->|Engages| G[AI Text Conversation]
    G -->|Qualifies Lead| H[AI Appointment Booking]
    end
    
    H -->|Syncs to| I[Contractor's Calendar]
    I -->|Contractor Visits| J[In-Person Estimate]
    J -->|Closes Deal| K[Paid Job / Revenue]
    
    K -->|Client Re-invests| A
```
